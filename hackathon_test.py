...
def update_item(item_id):
    ...
    # 2. Update fields if they are provided, otherwise keep the existing data
    item["name"] = update_data.get('name', item["name"])
    item["category"] = update_data.get('category', item["category"])
    
    # Safely handle float conversions if the user sent new numbers
    if 'quantity' in update_data:
        item["quantity"] = float(update_data['quantity'])
    if 'unit' in update_data:
        item["unit"] = update_data['unit']
    if 'reorder_threshold' in update_data:
        item["reorder_threshold"] = float(update_data['reorder_threshold'])
        
    # 3. Save to the JSON file
    save_data()
    return jsonify({"message": f"Successfully updated {item['name']}!"}), 200
...
def delete_item(item_id):
    ...
    # 2. Grab the name for the success message, then delete it from the dictionary
    deleted_name = Inventory[item_id]["name"]
    del Inventory[item_id]
    
    # 3. Save to the JSON file
    save_data()
    return jsonify({"message": f"Successfully deleted {deleted_name}!"}), 200
